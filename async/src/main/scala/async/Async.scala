...
object Async {
  ...
  def sequenceComputations[A, B](
    ...
    makeAsyncComputation1().flatMap(a =>  makeAsyncComputation2().map((a,_)))
  ...
  /**
    ...
    */
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] = makeAsyncComputation().recoverWith {
